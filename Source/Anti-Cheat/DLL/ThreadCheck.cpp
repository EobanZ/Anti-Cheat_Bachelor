...
void ThreadCheck::InitializeThreadCheck()
{
	...
	LdrInitializeThunk_ = reinterpret_cast<LdrInitializeThunk>(Utils::DetourFunc(reinterpret_cast<PBYTE>(LdrInitializeThunk_o), reinterpret_cast<PBYTE>(LdrInitializeThunk_t), 5));
...
void ThreadCheck::LdrInitializeThunk_t(PCONTEXT NormalContext, PVOID SystemArgument1, PVOID SystemArgument2)
{
	...
	printf("\t* Thread: %u - Suspended: %d\n", dwThreadId, Utils::isSuspendedThread(dwThreadId));
	...
	if (Utils::IsLoadedAddress(dwStartAddress))
		printf("# WARNING # IsLoadedAddress(dwStartAddress)\n");

	if (Utils::GetThreadOwnerProcessId(dwThreadId) != GetCurrentProcessId())
